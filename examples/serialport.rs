...
use serialport::{StopBits, Parity, SerialPortType, UsbPortInfo, SerialPortInfo};
...
fn select_port(ports: &[SerialPortInfo]) -> std::io::Result<&String> {
    ...
    for (idx, p) in ports.iter().enumerate() {
        let prod_str = if let SerialPortType::UsbPort(UsbPortInfo { product: Some(prod), .. }) = &p.port_type {
            format!("({prod})")
        } else {
            "".to_string()
        };
